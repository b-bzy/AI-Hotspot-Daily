...
| vendor_blogs | 大厂官方博客 | 最权威的官号一手来源。清单在 `配置.json` vendor_blogs。官方博客不是天天发，24h 窗口内常只有 0-4 条，是"某厂今天发了没"的权威捕获 |
...
## 大厂官方博客 RSS 实测（2026-07-10，vendor_blogs 源的依据）

比 X 镜像更权威——官方发布先上自家博客。逐个探测各厂 feed 路径：

**有 RSS 且在更新（已接入 vendor_blogs）**：OpenAI `openai.com/news/rss.xml` · Google DeepMind `deepmind.google/blog/rss.xml` · Google AI `blog.google/technology/ai/rss/` · Mistral `mistral.ai/rss.xml` · HuggingFace `huggingface.co/blog/feed.xml` · Apple ML `machinelearning.apple.com/rss.xml` · NVIDIA `blogs.nvidia.com/feed/`(需过滤,混游戏) · 微软研究院 `microsoft.com/en-us/research/feed/`(需过滤)。
**有 RSS 但已停更（不加）**：Qwen `qwenlm.github.io/blog/index.xml`(最新一篇 291 天前,阿里已把 Qwen 博客迁走)。
**无 RSS（官网不提供）**：Anthropic(所有路径 404,靠 zpravobot @AnthropicAI 覆盖) · Meta AI(ai.meta.com 无 feed;engineering.fb.com 有但偏系统工程,Meta AI 新闻靠媒体覆盖) · DeepSeek · Cohere · Stability AI · xAI · ElevenLabs · Runway。
**可选未加**（authoritative 但偏小众/低频）：Amazon Science `amazon.science/index.rss` · Databricks `databricks.com/feed` · Together AI `together.ai/blog/rss.xml` · 微软AI blog(仅 news.microsoft.com 泛企业新闻可用)。想加直接往 `配置.json` vendor_blogs 追加即可。
